...
class TestModel(ModelTestCase):
    ...
    def test_context_fork(self):
        ...
        with context.fork(chain_id=10) as cc:
            self.assertEqual(
                credmark.cmf.model.ModelContext.current_context(), cc)
        ...
        with context.fork(block_number=context.block_number - 10) as cc1:
            with cc1.fork(block_number=cc1.block_number - 10) as cc2:
                with cc2.fork(block_number=cc2.block_number - 10) as cc3:
                    self.assertEqual(
                        credmark.cmf.model.ModelContext.current_context(), cc3)
        ...
        self.assertEqual(current_network, context.network)
        self.assertEqual(current_block, context.block_number)

    def test_context_fork_contract(self):
        for symbol in ['USDC', 'CRV']:
            ...
            with context.fork(block_number=current_block - 2_000_000):
                ...
                self.assertEqual(usdc.total_supply_scaled,
                                 usdc.total_supply / (10**usdc.decimals))
                ...
                with context.fork(block_number=current_block - 4_000_000):
                    ...
                    self.assertEqual(usdc.functions.name().call(), usdc.name)
                    self.assertEqual(
                        usdc.functions.symbol().call(), usdc.symbol)
                    self.assertEqual(
                        usdc.functions.decimals().call(), usdc.decimals)

            self.assertEqual(usdc.total_supply_scaled,
                             current_total_supply_scaled)
