...
class Web3Batch(ABC):
    ...
    def _divide_chunks(
        self, big_list: Sequence[T], batch_size: int | None = None
    ) -> Iterator[Sequence[T]]:
    ...
    def _decode_data(self, fn_name: str | None, output_types: Sequence[str], return_data: bytes):
    ...
    def _decode_results(
        ...
        *,
        ...
        for payload, multicall_result in zip(payloads, encoded_results):
            ...
            try:
                data = (
                    self._decode_data(payload["fn_name"], payload["output_type"], encoded_data)
    ...
    def _chunk_process_payloads(self, payloads: list[Payload]) -> list[MulticallResult]:
        results: list[MulticallResult] = []
        for chunk in self._divide_chunks(payloads):
            results.extend(self._process_payloads(list(chunk)))
        return results
    ...
    ) -> tuple[MulticallDecodedResult[Any], ...]: ...
    ...
    def call(
        ...
        *,
        ...
        unwrap: bool = False,
        unwrap_default: Any = None,
        return_type: type[U] = tuple[Any, ...],  # pylint: disable=unused-argument
    ) -> U:
        results: list[MulticallDecodedResult] = []
        for chunk in self._divide_chunks(contract_functions):
            payloads = self._build_payload(chunk)
            encoded_results = self._process_payloads(payloads)
            results.extend(
                self._decode_results(payloads, encoded_results, require_success=require_success)
            )
    ...
    def call_same_function(
        ...
        *,
        ...
        fallback_functions: Sequence[ContractFunction] | None = None,
        unwrap: Literal[False],
    ) -> list[MulticallDecodedResult[Any]]: ...
    ...
    def call_same_function(  # pylint: disable=too-many-locals
        ...
        *,
        ...
        unwrap_default: Any = None,
        return_type: type[T] | Any = Any,
    ) -> list[T | None] | list[MulticallDecodedResult[T]]:  # pylint: disable=unused-argument
        results: list[MulticallDecodedResult[T]] = []
        for chunk in self._divide_chunks(contract_addresses):
        ...
        failed = [
            (idx, contract_addresses[idx])
            for idx, decoded_result in enumerate(results)
