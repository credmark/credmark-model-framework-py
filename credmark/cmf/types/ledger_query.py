...
class LedgerQueryBase(contextlib.AbstractContextManager):
    ...
    def _gen_model_input(
        ...
        if group_by is not None and not omit_group_by_columns:
            if columns == []:
                columns = None
            ...
            columns = [c for c in group_by if c in self.columns and c not in aggregates_names]  # type: ignore
        ...
        if columns is None:
            ...
            raise InvalidQueryException(model_slug, f"{columns=} needs to be a list of string.")
        else:
