...
class Token(Contract):
    ...
    _meta: TokenMetadata = PrivateAttr(default_factory=lambda: Token.TokenMetadata())  # pylint: disable=unnecessary-lambda
    ...
    def __new__(cls, *args, **data):
        ...
        token_data = get_token_from_configuration(
            chain_id=context.chain_id, symbol=symbol, address=address, is_native_token=True
        )
    ...
    def __init__(self, *args, **data):
        ...
        if "address" not in data and "symbol" in data:
            ...
            data["meta"]["wrapped"] = (
                Address(token_data["wrapped"]) if "wrapped" in token_data else None
            )
    ...
    def try_erc20_property(self, prop_name):
        try:
            ...
            raise ModelDataError(
                ...
                f" proxied by {self.proxy_for.address}"
                if self.proxy_for is not None
                else ""
            ) from err
    ...
    def symbol(self) -> str:
        self._load()
        current_block = int(
            credmark.cmf.model.ModelContext.current_context().web3.eth.default_block
        )
    ...
    def decimals(self) -> int:
        ...
        if not self._meta.set_loaded:
            self._meta.decimals = self._meta.get_cache(
                field="decimals", chain_id=current_chain_id, block_number=current_block
            )
    ...
    def name(self) -> str:
        ...
        current_chain_id = credmark.cmf.model.ModelContext.current_context().chain_id
        if not self._meta.set_loaded:
            self._meta.name = self._meta.get_cache(
                field="name", chain_id=current_chain_id, block_number=current_block
            )
    ...
    def total_supply(self) -> int:
        ...
        current_chain_id = credmark.cmf.model.ModelContext.current_context().chain_id
        if not self._meta.set_loaded:
            self._meta.total_supply = self._meta.get_cache(
                field="total_supply", chain_id=current_chain_id, block_number=current_block
            )
